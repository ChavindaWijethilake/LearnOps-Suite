      ...
      ═══════════════════════════════════════════ */}
            ...
            >
                ...
                <nav className="flex-1 px-5 py-6 space-y-2 overflow-y-auto">
                    {/* Main Nav */}
                    <NavLabel>Main Command</NavLabel>
                    {[
                    ...
                    {/* Quick Access */}
                    <NavLabel>Directives</NavLabel>
                    {[
                        { icon: Library, label: 'Global Library', id: 'Library', href: '/library' },
                        { icon: Bookmark, label: 'Saved Assets', id: 'Saved', href: '/saved' },
                        { icon: Clock, label: 'Session History', id: 'Recent', href: '/recent' },
                        { icon: Upload, label: 'Dispatch Resource', id: 'Submit', href: '/submit' },
                    ].map((item) => <SidebarLink key={item.id} item={item} activeNav={activeNav} setActiveNav={setActiveNav} />)}

                    {/* Secondary */}
                    <NavLabel>External</NavLabel>
                    {[
                        { icon: Globe, label: 'Public Site', id: 'Public', href: 'http://localhost:3000' },
                        { icon: CreditCard, label: 'Billing Portal', id: 'Billing', href: 'http://localhost:3000/billing' },
                        { icon: GraduationCap, label: 'Professor Portal', id: 'Professor', href: 'http://localhost:3000/professor' },
                        { icon: Shield, label: 'Admin Center', id: 'Admin', href: 'http://localhost:3000/status' },
                    ].map((item) => <SidebarLink key={item.id} item={item} activeNav={activeNav} setActiveNav={setActiveNav} />)}
                </nav>
      ...
      ═══════════════════════════════════════════ */}
            <main className="flex-1 md:ml-[280px] min-h-screen flex flex-col relative z-10">
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap items-center gap-2 mt-1">
                                <span className="text-[10px] md:text-[11px] font-bold text-emerald-500/80 uppercase tracking-[0.2em]">Operational Protocol v4.0.1</span>
                                <span className="hidden md:block w-1 h-1 bg-slate-700" style={{ borderRadius: '50%' }} />
                                <span className="hidden md:block text-[11px] font-medium text-slate-500">Security: Verified</span>
                                <span className="w-1 h-1 bg-slate-700 md:hidden" style={{ borderRadius: '50%' }} />
                                <span className="text-[10px] md:text-[11px] font-medium text-emerald-400/60 transition-all animate-pulse">Last Synced: {lastSynced || '--:--:--'}</span>
                            </div>
                        </div>
                    </div>
                ...
                <div className="p-12 space-y-12">
                    ...
                    {/* ─────────────────────────────────────────
              3. FEATURED RESOURCES (Responsive)
          ───────────────────────────────────────── */}
                    <section className="flex flex-col lg:grid lg:grid-cols-12 gap-8 lg:gap-10">
                        {/* Primary Card (65 %) */}
                        <div
                            className="lg:col-span-8 group relative overflow-hidden cursor-pointer hover:-translate-y-1.5 transition-all duration-700"
                            style={{
                                borderRadius: '22px',
                                background: 'rgba(255,255,255,0.04)',
                                border: '1px solid rgba(255,255,255,0.05)',
                        ...
                        >
                            ...
                            <div className="relative z-10 p-8 md:p-12 h-full flex flex-col justify-end">
                                ...
                                <h3 className="text-2xl md:text-[34px] font-bold text-white mb-4 tracking-tight leading-[1.1] group-hover:text-emerald-400 transition-colors duration-700">
                                    Industrial Cloud Architecture <br className="hidden md:block" /> Orchestration Protocol 9.2
                                </h3>
                                <p className="text-sm md:text-[17px] text-slate-400 leading-relaxed max-w-xl mb-8 md:mb-10">
                                ...
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                                    <div className="flex items-center gap-5">
                                        <div className="w-10 h-10 md:w-12 md:h-12 bg-slate-900 flex items-center justify-center overflow-hidden" style={{ borderRadius: '9999px', border: '2px solid rgba(255,255,255,0.10)', boxShadow: '0 8px 20px rgba(0,0,0,0.3)' }}>
                                        ...
                                        <div>
                                            <span className="text-sm md:text-base font-bold text-white tracking-tight block">Technical Operations</span>
                                            <span className="text-[10px] md:text-[11px] text-slate-500 font-black uppercase tracking-widest">Core Engineering Hub</span>
                                        </div>
                                    </div>

                                    <button
                                        className="w-fit text-white font-bold text-xs md:text-sm flex items-center gap-3 hover:-translate-y-1 active:scale-95 transition-all duration-500"
                                        style={{
                                            padding: '20px 48px',
                                            borderRadius: '12px',
                                            background: 'linear-gradient(135deg,#10B981,#6366F1)',
          ...
          ───────────────────────────────────────── */}
                    <SectionBlock title="Browse by Resource Type" subtitle="Explore resources organized by content format.">
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 md:gap-5">
                            ...
                            <NavCard href="/resources/presentations" icon={Presentation} label="Presentation Slides" count={56} accent="#EC4899" />
                            <NavCard href="/resources/downloads" icon={Download} label="Downloadable Materials" count={45} accent="#14B8A6" />
                        </div>
                    </SectionBlock>
          ...
          ───────────────────────────────────────── */}
                    <SectionBlock title="Academic Archives" subtitle="Browse past academic materials by type.">
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
                            ...
                            <NavCard href="/archives/coursework-topics" icon={List} label="Past Coursework Topics" count={89} accent="#8B5CF6" />
                            <NavCard href="/archives/sample-projects" icon={FolderOpen} label="Sample Projects" count={67} accent="#06B6D4" />
                            <NavCard href="/archives/final-year-projects" icon={GraduationCap} label="Final Year Projects Archive" count={145} accent="#10B981" />
                            <NavCard href="/archives/assignments" icon={ClipboardList} label="Assignment Submissions Archive" count={234} accent="#D946EF" />
                        </div>
                    </SectionBlock>
          ...
          ───────────────────────────────────────── */}
                    <SectionBlock title="Project Repository" subtitle="Discover student and faculty projects across disciplines.">
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 md:gap-5">
                            ...
                            <NavCard href="/projects/open-source" icon={Github} label="Open Source Contributions" count={18} accent="#14B8A6" />
                        </div>
                    </SectionBlock>
                    ...
                    <section className="space-y-10">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10">
                            {categories.map((cat) => (
                                <div
                                    key={cat.id}
                                    className="group relative p-8 md:p-10 overflow-hidden cursor-pointer hover:-translate-y-1 transition-all duration-700"
                                    style={{
                                        borderRadius: '22px',
                                ...
                                >
                                    ...
                                    <div className="flex items-start justify-between mb-8 md:mb-10 relative z-10">
                                        <div
                                            className={`w-14 h-14 md:w-16 md:h-16 flex items-center justify-center bg-gradient-to-br ${cat.gradient} group-hover:scale-110 group-hover:rotate-3 transition-all duration-700`}
                                            style={{ borderRadius: '18px', border: '1px solid rgba(255,255,255,0.10)', boxShadow: '0 8px 20px rgba(0,0,0,0.2)' }}
                                        >
                                            <cat.icon className="w-6 h-6 md:w-8 md:h-8 text-white" />
                                        </div>
                                        <div className="text-right">
                                            <span className="text-[10px] md:text-[11px] font-black text-slate-600 uppercase tracking-widest block">Volume</span>
                                            <span className="text-xl md:text-[28px] font-bold text-white mt-1 md:mt-2 tabular-nums block" style={{ fontFamily: 'monospace' }}>{cat.count}</span>
                                        </div>
                                    </div>

                                    <h4 className="text-lg md:text-[20px] font-bold text-white group-hover:text-emerald-400 transition-colors duration-500 relative z-10">{cat.name}</h4>
                                    <p className="text-xs md:text-sm text-slate-500 mt-2 md:mt-3 leading-relaxed max-w-[340px] font-medium relative z-10">{cat.desc}</p>
                                </div>
                            ))}
                        </div>
                    </section>
          ...
          ───────────────────────────────────────── */}
                    <div className="grid grid-cols-12 gap-12">
                        ...
                        <div className="col-span-12 lg:col-span-7">
                            ...
                            >
                                ...
                                <div className="relative z-10 px-8">
                                    ...
                                    <h3 className="text-2xl md:text-[28px] font-bold text-white tracking-tight leading-tight">Elevate Institutional Knowledge</h3>
                                    <p className="text-slate-400 text-sm md:text-base mt-4 font-medium leading-relaxed max-w-lg">
                                    ...
                                    <div className="flex flex-col sm:flex-row sm:items-center gap-8 mt-10">
                                        <button
                                            className="text-[#0F172A] font-bold text-xs md:text-sm flex items-center justify-center gap-3 hover:-translate-y-1 active:scale-95 transition-all duration-500"
                                            style={{
                                                padding: '20px 48px',
                                                borderRadius: '12px',
                                                background: '#10B981',
                        ...
                        {/* Quick Links + Stats */}
                        <div className="col-span-12 lg:col-span-5 space-y-8 md:space-y-10">
                            {/* Quick Links */}
                            <div
                                style={{
                                    borderRadius: '22px',
                                    background: 'rgba(255,255,255,0.03)',
                                    border: '1px solid rgba(255,255,255,0.05)',
                                    padding: '24px md:padding: 40px',
                                    paddingTop: '32px',
                                    paddingBottom: '32px',
                                    boxShadow: '0 10px 30px rgba(0,0,0,0.25)',
                                }}
                            >
                                <h4 className="px-4 text-[11px] font-black text-slate-500 uppercase tracking-[0.2em] mb-8 md:mb-10 flex items-center gap-4">
                                ...
                                <div className="space-y-3 md:space-y-4 px-4">
                                    {quickLinks.map((link) => (
                                        <div
                                            key={link.id}
                                            className="group flex items-center gap-4 md:gap-6 relative overflow-hidden cursor-pointer hover:bg-white/5 transition-all duration-500"
                                        ...
                                        >
                                            ...
                                            <div
                                                className="w-12 h-12 md:w-14 md:h-14 flex items-center justify-center text-slate-400 group-hover:text-emerald-400 group-hover:rotate-6 transition-all"
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <h5 className="text-sm md:text-[16px] font-bold text-white group-hover:text-emerald-300 transition-colors leading-tight">{link.title}</h5>
                                                <p className="text-[10px] md:text-[11px] text-slate-500 font-bold uppercase tracking-widest mt-1.5 truncate">{link.desc}</p>
                                            </div>
                                            <div
                                                className="hidden sm:flex w-8 h-8 items-center justify-center opacity-0 group-hover:opacity-100 -translate-x-4 group-hover:translate-x-0 transition-all duration-500"
          ...
          ───────────────────────────────────────── */}
                                <div className="col-span-12">
                                    <div
                                        ...
                                        style={{
                                            borderRadius: '24px',
                                            padding: '32px md:padding: 40px',
                                            background: 'rgba(255,255,255,0.02)',
                                            border: '1px solid rgba(255,255,255,0.05)',
                                            boxShadow: '0 20px 40px rgba(0,0,0,0.2)'
                                        }}
                                    >
                                        <div className="absolute inset-0 bg-gradient-to-r from-indigo-500/5 to-emerald-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-1000" />
                                        ...
                                        <div className="relative z-10 flex items-center justify-between">
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2 mt-4">
                                                    ...
                                                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Contributors</span>
                                                </div>
                                            </div>
                                            <div

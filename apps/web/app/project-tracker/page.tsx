...
export default function DashboardPage() {
  ...
  const deleteProject = async (id: string) => {
    ...
    try {
      const response = await fetch(`/api/project-tracker/projects/${id}`, { method: 'DELETE' });
      if (response.ok) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        <Link href="/project-tracker/projects/new">
      ...
      {isLoading ? (
        ...
        <div className="grid gap-6">
          {projects.map((project) => (
            <Link key={project.id} href={`/project-tracker/projects/${project.id}`}>
